...
export function AskAiLogo({
  id,
  name,
  className,
}: Pick<AskAiTarget, "id" | "name"> & { className?: string }) {
  const path = brandPaths[id as keyof typeof brandPaths]
...
export function createAskAiTargets(prompt: string): AskAiTarget[] {
...
export interface AskAiProps extends Omit<
...
> {
  ...
  prompt: string
  subject: string
  targets?: readonly AskAiTarget[]
}

type CopyState = "idle" | "copied" | "error"

export const AskAi = React.forwardRef<HTMLDivElement, AskAiProps>(
  function AskAi(
    {
      className,
      copyLabel = "Copy prompt",
      description = "Choose an assistant, or copy the prompt for another one.",
      onPromptCopied,
      prompt,
      subject,
      targets,
    ...
    const resolvedTargets = React.useMemo(
      () => targets ?? createAskAiTargets(prompt),
      [prompt, targets]
    )
    ...
    return (
      <div
        className={cn(
          "bg-card text-card-foreground border-border flex flex-col gap-4 rounded-xl border p-4 sm:flex-row sm:items-center sm:justify-between",
          className
        )}
        data-slot="ask-ai"
        ref={forwardedRef}
        ...
        >
          {resolvedTargets.map((target) => (
            <a
              ...
              target="_blank"
              title={`Ask ${target.name}`}
            >
              <AskAiLogo id={target.id} name={target.name} />
            </a>
          ))}
